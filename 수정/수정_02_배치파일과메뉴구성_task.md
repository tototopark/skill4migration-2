...
- [ ] **2단계: 공통 비즈니스 로직 및 bcrypt 호환 처리 구현**
  - [ ] `functions.inc.php` 및 `1.php`에 정의된 핵심 룰 이전 (`src/domain.py`, `src/service.py`)
  - [ ] PHP `$2y$` 시작 bcrypt 패스워드 호환 검증기 구현
  - [ ] IP 및 디바이스(private key) 검증 로직 구현
