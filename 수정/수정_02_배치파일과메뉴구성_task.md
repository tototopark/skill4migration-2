...
- [x] **4단계: 개발자 진단용 힌트(DevHints) 및 스위치 추가**
  - [x] 화면 하단에 바인딩할 API, 파일 경로 메타데이터 수집 컴포넌트 설계 (api 엔드포인트 `/dev/hints`로 구현)
  - [x] 환경 변수 (`SHOW_DEV_HINTS`) 연동 스위치 개발 (활성/비활성 Render None 분기 처리)
- [ ] **5단계: Gate 2 회귀 대조 검증**
